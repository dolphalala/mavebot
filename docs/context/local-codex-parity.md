...
for mavebot's Discord `#codex` control channel.
...
- Nearby Discord channel messages and files may be included as background
  context for references, screenshots, and collaboration.
...
- Local attachment paths under `/shared/codex-worker/context/discord-files/`
  are part of the active prompt when present and should be inspected if
  relevant. Supported image files are passed to Codex with `--image` so remote
  screenshot work has the same visual-input path as a local Codex session.
