...
test('buildCodexWorkerPrompt treats ClashKing and ClashPerk asks as product discovery', () => {
  ...
  assert.match(prompt, /\/history player is the first reporting surface on that store/);
  assert.match(prompt, /Future \/roster, \/warstats, \/activity, and \/summary work should build from the same store/);
