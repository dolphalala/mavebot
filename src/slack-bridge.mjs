...
export function buildCodexPromptText({
  ...
  const parts = [
    ...
    '',
    'Mavebot legacy Slack session contract:',
    '- Treat this as one turn forwarded from the legacy #bot Slack bridge. Discord #codex is the primary remote session.',
    ...
    '- Read docs/context/discord-session.md next for durable channel memory, current goals, decisions, and open threads.',
    '- If the user asks to reset, start over, or create a new session, add a new dated section in docs/context/discord-session.md and use that as the active context.',
    '- Otherwise keep continuity by appending durable facts, decisions, and next steps to docs/context/discord-session.md whenever the turn changes what future Codex runs should know.',
