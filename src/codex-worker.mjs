...
function promptHeader(job) {
  ...
  return [
    ...
    '- The current Clash data-collection entry point is /track player, /track clan, and /track status backed by /shared/clash-history.json. /history player is the first reporting surface on that store. Future /roster, /warstats, /activity, and /summary work should build from the same store before adding parallel state.',
